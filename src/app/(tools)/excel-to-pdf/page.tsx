...
export default function ExcelToPdf() {
  ...
  const convertExcel = async () => {
    ...
    setStatus("Reading spreadsheet file...");

    try {
      // Dynamically import xlsx and pdfmake to bypass SSR issues
      ...
      setStatus("Parsing Excel sheets...");
      const workbook = XLSX.read(new Uint8Array(arrayBuffer), { type: "array" });
      const firstSheetName = workbook.SheetNames[0];
      const worksheet = workbook.Sheets[firstSheetName];

      // Convert sheet to 2D JSON array (array of arrays)
      ...
      setStatus("Formatting document tables...");
      // Handle column structure irregularities
      let maxCols = 0;
      rawData.forEach((row) => {
      ...
      // Align cells and pad missing columns to prevent pdfmake crashes
      ...
      const docDefinition = {
        // Landscape orientation accommodates wider Excel sheets
        pageOrientation: "landscape" as const,
        content: [
          {
            table: {
              headerRows: 1,
              dontBreakRows: true, // handle page breaks gracefully
    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-6 md:p-12 text-zinc-900 dark:text-zinc-50 font-mono">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {!file ? (
          <DropZone
            accept={[".xlsx", ".xls"]}
            label="Drag & Drop Excel Spreadsheet"
